...
export default function ContactClient({ initialImages }: ContactClientProps) {
  ...
  const getImageUrl = (key: string, fallback: string) => {
    if (!Array.isArray(initialImages)) return fallback;
    const match = initialImages.find(x => x && x.page === "contact" && x.key === key);
